...
Checks cover issued editing and PDF downloads, private-note exclusion, cancellation, stale/conflicting writes, permission failures, required fields, paid balance limits, remove/restore, cloned metadata, and mobile layout. Live Supabase authorization and deployment must be verified in the user's normal environment; these tests do not alter real invoices.
